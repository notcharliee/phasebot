...
export default Functions.clientEvent({
  ...
  async execute(client, channel) {
    ...
    const guildAutoPartnerSchema = await Database.AutoPartners.findOne({ guild: (channel as Discord.NonThreadGuildBasedChannel).guildId }) // The partnership schema of the current guild.
    const partnerAutoPartnerSchemas = await Database.AutoPartners.find({ partners: { $elemMatch: { channelId: channel.id } } }) // Any partnership schemas that had the guild as a partner.
