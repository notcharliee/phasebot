...
export async function alertDevs (data: { title: string, description?: string, type: 'message' | 'warning' | 'error' }) {
  ...
  let emoji: string = '⚠️ '
  if (data.type == 'message') emoji = Enums.PhaseEmoji.Announce
  else if (data.type == 'error') emoji = Enums.PhaseEmoji.Failure
  ...
  const webhookAlert = await webhookClient.send({
    embeds: [
      new Discord.EmbedBuilder()
        ...
        .setColor(data.type == 'message' ? Enums.PhaseColour.Primary : data.type == 'warning' ? Enums.PhaseColour.Warning : Enums.PhaseColour.Failure)
        .setTimestamp()
        .setFooter({ text: Constants.isDevEnv ? 'Phase [Alpha]' : 'Phase [Production]' })
    ]
  })
 ...
 * @param error The error string (common errors found in {@link Enums.PhaseError}).
 */
export function clientError <deffered = false> (
  ...
  const interactionReplyOptions: Discord.InteractionReplyOptions = {
    ...
    embeds: [
      new Discord.EmbedBuilder()
        .setColor(Enums.PhaseColour.Failure)
        .setDescription(error)
        .setTitle(Enums.PhaseEmoji.Failure + title)
    ],
    ephemeral
  }

  const throwError = (error: any) => Functions.alertDevs({
...
export function clientSlashCommand ({ data, permissions, execute }: {
  data: Types.SlashCommandData<true>,
  permissions?: Types.SlashCommandPermissions,
  execute: (
...
}): Types.EventFile<'interactionCreate'> {
