...
export default Functions.clientSlashCommand({
  ...
  async execute(client, interaction) {
    ...
    const guildPartnerSchema = await Database.AutoPartners.findOne({ guild: interaction.guildId }) ?? await new Database.AutoPartners({
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case 'add': {
        ...
        if (!guildPartnerSchema.channel || !guildPartnerSchema.advert) return Functions.clientError<true>(
          interaction,
          'No can do!',
        ...
        const invitePartnerSchema = await Database.AutoPartners.findOne({ invites: { $elemMatch: { code: inviteCode } } })

        if (!invitePartnerSchema || invitePartnerSchema.invites.find(invite => invite.code == inviteCode)!.expires <= new Date().getTime().toString()) return Functions.clientError<true>(
          interaction,
          'No can do!',
          'The invite code is either invalid or has expired.'
        )

        if (invitePartnerSchema.guild == interaction.guildId) return Functions.clientError<true>(
          interaction,
          'No can do!',
          'You can\'t partner with your own server!'
        )

        if (guildPartnerSchema.partners.find(partner => partner.guildId == invitePartnerSchema!.guild)) return Functions.clientError<true>(
          interaction,
          'No can do!',
          ...
          interaction.editReply({
            embeds: [
              new Discord.EmbedBuilder()
                ...
                .setTitle(Enums.PhaseEmoji.Success + 'Partnership Created')
            ],
          })
        ...
        interaction.reply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setTitle(Enums.PhaseEmoji.Success + 'Channel Set')
          ],
        })
      ...
      case 'invite': {
        ...
        // Creates partner invite code, then saves it to the database.

        async function newInviteCode() {

          let inviteCode = createInvideCode()
      
          while (await Database.AutoPartners.findOne({ invites: { $elemMatch: { code: inviteCode } } })) {
        ...
        interaction.editReply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setTitle(Enums.PhaseEmoji.Success + 'Invite Code Created')
          ],
        })
      ...
      case 'remove': {
        ...
        const invitePartnerSchema = await Database.AutoPartners.findOne({ guild: partnerGuildId })
        ...
        if (!invitePartnerSchema || !guildPartnerSchema) return Functions.clientError<true>(
          interaction,
          'No can do!',
        ...
        interaction.editReply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setTitle(Enums.PhaseEmoji.Success + 'Partnership Terminated')
          ],
        })
