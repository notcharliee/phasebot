  ...
  async execute(client, interaction) {
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case 'set': {
        ...
        const levelsSchema = await Database.Levels.findOne({ guild: interaction.guildId })

        if (!levelsSchema) return Functions.clientError<true>(
          interaction,
          'No can do!',
          'Make sure the Levels & XP module is enabled, then try again.'
        )
        ...
        if (userIndex == -1) return Functions.clientError<true>(
          interaction,
          'No can do!',
        ...
        interaction.editReply({
          embeds: [
            ...
            .setTitle(Enums.PhaseEmoji.Success + 'Level Data Set')
          ],
        })
